...
import { AppSettings, CoupletData, DeepSeekResponse, ParsedName } from '../types';

const callDeepSeek = async (settings: AppSettings, systemPrompt: string, userContent: string): Promise<string> => {
...
export const parseName = async (name: string, settings: AppSettings): Promise<ParsedName> => {
  const jsonStr = await callDeepSeek(settings, PROMPT_NAME_PARSING, `输入：“${name}”`);
...
export const generateCouplet = async (parsedName: ParsedName, settings: AppSettings): Promise<CoupletData> => {
  ...
  const jsonStr = await callDeepSeek(settings, PROMPT_COUPLET_GENERATION, `解析数据：${parsedNameStr}`);
